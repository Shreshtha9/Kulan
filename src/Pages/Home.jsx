  ...
  {/* Yellow wave background */}
   <span className="absolute inset-0 bg-gradient-to-r from-[#ffd800] to-[#ffd800]/60 blur-sm translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-500 ease-in-out z-0" />
    ...
    {/* solution */}
      ...
      {/* Custom Cursor */}
      <div 
        className={`fixed w-6 h-6 rounded-full pointer-events-none z-50 transition-all duration-75 ease-out border-2 border-[#f5d78c] mix-blend-exclusion ${isPointer ? 'scale-150' : 'scale-100'}`}
        style={{ 
      ...
      />
      <div 
        className={`fixed w-2 h-2 bg-[#f5d78c] rounded-full pointer-events-none z-50 transition-transform duration-150 ease-out ${isPointer ? 'opacity-0' : 'opacity-100'}`}
        style={{ 
          left: `${cursorPosition.x}px`, 
          top: `${cursorPosition.y}px`,
          transform: 'translate(-50%, -50%)'
